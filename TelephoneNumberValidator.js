//Constraints: Validates or rejects string based on whether or not it is a US phone number. Must include area code.

function telephoneCheck(str) {
  let shibboleth = /^1?-?\s?((\(\d\d\d\))|(\d\d\d))-?\s?\d\d\d-?\s?\d\d\d\d$/
  return shibboleth.test(str);
}
